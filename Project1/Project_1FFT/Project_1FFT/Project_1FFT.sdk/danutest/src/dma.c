...
int intr_init(XScuGic_Config *cfg_ptr)
{
	...
	XScuGic_SetPriorityTriggerType(&axi_intc, XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR, 0xA0, 0x3);
	XScuGic_SetPriorityTriggerType(&axi_intc, XPAR_FABRIC_AXI_DMA_0_MM2S_INTROUT_INTR, 0xA8, 0x3);

	// Connect handlers
	status = XScuGic_Connect(&axi_intc, XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR, (Xil_InterruptHandler)s2mm_isr, &axi_dma);
	if (status != XST_SUCCESS)
	{
		xil_printf("ERROR! Failed to connect s2mm_isr to the interrupt controller \r\n", status);
		return status;
	}
	status = XScuGic_Connect(&axi_intc, XPAR_FABRIC_AXI_DMA_0_MM2S_INTROUT_INTR, (Xil_InterruptHandler)mm2s_isr, &axi_dma);
	if (status != XST_SUCCESS)
	{
	...
	// Enable interrupts
	XScuGic_Enable(&axi_intc, XPAR_FABRIC_AXI_DMA_0_S2MM_INTROUT_INTR);
	XScuGic_Enable(&axi_intc, XPAR_FABRIC_AXI_DMA_0_MM2S_INTROUT_INTR);
